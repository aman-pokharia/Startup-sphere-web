...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: ArrayBufferView | ArrayBuffer, usage: number) {
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: InfiniteMenuItem[], onActiveItemChange: (index: number) => void, onMovementChange: (isMoving: boolean) => void, onInit: ((instance: InfiniteGridMenu) => void) | null = null, scale: number = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  resize() {
    ...
    this.viewportSize = vec2.set(vec2.create(), canvas.clientWidth, canvas.clientHeight);
  ...
  private findNearestVertexIndex(): number {
    ...
    return nearestVertexIndex !== null ? nearestVertexIndex : 0;
  }
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      // Do nothing since we're not displaying active item info
    };
